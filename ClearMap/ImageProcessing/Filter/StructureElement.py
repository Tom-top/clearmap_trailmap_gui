...
def structureElement(setype = 'Disk', sesize = (3,3), ndim = None):
    """Creates specific 2d and 3d structuring elements
      
    Arguments:
        setype : str
            structure element type, see :ref:`StructureElementTypes`
        sesize : array or tuple
    ...
    sesize = np.array([sesize]).flatten();   
    
    if ndim is None:
      ndim = len(sesize);
    else:
      sesize = np.pad(sesize[:ndim], (0, max(0, ndim - len(sesize))), 'wrap');
      
      
    if ndim == 1:
      sesize = (sesize[0], sesize[0]);
      ndim = 2;
...
def structureElement3D(setype = 'Disk', sesize = (3,3,3)):
    ...
    if len(sesize) != 3:
        raise RuntimeError('structureElement3D: sesize is not 3d');
