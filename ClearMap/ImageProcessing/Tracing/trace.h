...
template <typename dtype_source_t, typename index_t>
class Tracer {

  public:
    //image data
    dtype_source_t* source;
    index_t shape_x, shape_y, shape_z;
    index_t stride_x, stride_y, stride_z;

    //bool use_tubeness;
    dtype_source_t* tubeness;
    ...
    double tubeness_multiplier;
    double minimal_tubeness;
  ...
  public:
    //constructors
//    Tracer(dtype_source_t* source_, index_t shape_x_, index_t shape_y_, index_t shape_z_,
//                                    index_t stride_x_,index_t stride_y_,index_t stride_z_,
//           dtype_source_t* tubeness_, Scale scale_ = one) {
...
//      }
    ...
    void setDefault() {
      ...
      tubeness_multiplier = 4.0;

      minimal_tubeness = 0.2;
    ...
    void setData(dtype_source_t* source_, index_t shape_x_, index_t shape_y_, index_t shape_z_,
                                          index_t stride_x_,index_t stride_y_,index_t stride_z_,
                 dtype_source_t* tubeness_) {
      source = source_;
      shape_x = shape_x_;
      shape_y = shape_y_;
      shape_z = shape_z_;
  ...
  public:
    ...
    virtual double costMovingTo(const Point& point, int dx, int dy, int dz) {
      ...
      //if (use_tubeness) {
      double measure = tubeness[index];
      if (measure <= minimal_tubeness) { measure = minimal_tubeness; }
      cost = 1 / (tubeness_multiplier * measure);
      //std::cout << "tubeness=" << measure << std::endl;
      //} else {
...
//                    }
  ...
  int run(dtype_source_t* source_, index_t shape_x_, index_t shape_y_, index_t shape_z_,
                                    index_t stride_x_,index_t stride_y_,index_t stride_z_,
           dtype_source_t* tubeness_,
...
template <typename dtype_source_t, typename index_t, typename dtype_mask_t>
class TracerToMask : public Tracer<dtype_source_t, index_t> {
