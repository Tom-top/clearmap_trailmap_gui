"""
Module to trace paths between two points in a 3d image
"""
...
def trace(source, tubeness, start, stop, 
...
#        raise RuntimeError('Tubeness array not c-contigous');    
    ...
    path = code.trace(source, tubeness, np.array(start), np.array(stop), 
...
def traceToMask(source, tubeness, start, mask, 
                costPerDistance = 1.0, minimumCostPerDistance = 1/60.0,
                tubenessMultiplier = 4.0, minimalTubeness = 0.1,
                returnQuality = False,
                maxSteps = None, verbose = False):
