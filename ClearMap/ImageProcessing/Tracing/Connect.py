...
def plotData(data, skel, binary, ends = None, isolated = None, replot = True):
  ...
  if replot:
    try:
      ...
      viewer = dv.dualPlot(data, img);
  else:
    return dv.dualPlot(data, img);
...
def plotTracingResult(path, data_nbh, mask_nbh, center, radius, tubeness_nbh, skeleton = None, distance_nbh = None):
  ...
  try:
    ...
    if distance_nbh is not None:
      viewer = dv.multiPlot([bin_nbh, img_nbh, tubeness_nbh, distance_nbh])
    else:
      viewer = dv.multiPlot([bin_nbh, img_nbh, tubeness_nbh])
...
if __name__ == "__main__":
  ...
  try:
    ...
    con.viewer = dv.dualPlot(mask_img, data_img);
