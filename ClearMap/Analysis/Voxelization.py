# -*- coding: utf-8 -*-
"""
Converts point data into voxel image data for visulalization and analysis
"""
...
def voxelize(points, dataSize = None, sink = None, voxelizeParameter = None,  method = 'Spherical', size = (5,5,5), weights = None):
    ...
    if method.lower() == 'spherical':
        if weights is None:
            data = vox.voxelizeSphere(points, dataSize[0], dataSize[1], dataSize[2], size[0], size[1], size[2]);
        else:
            data = vox.voxelizeSphereWithWeights(points, dataSize[0], dataSize[1], dataSize[2], size[0], size[1], size[2], weights);
           
    elif method.lower() == 'rectangular':
        if weights is None:
            data = vox.voxelizeRectangle(points, dataSize[0], dataSize[1], dataSize[2], size[0], size[1], size[2]);
        else:
            data = vox.voxelizeRectangleWithWeights(points, dataSize[0], dataSize[1], dataSize[2], size[0], size[1], size[2], weights);
