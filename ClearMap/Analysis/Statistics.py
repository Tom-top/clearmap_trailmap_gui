...
def PearsonsVoxelization(group1, group2):
    """t-Test on differences between the individual voxels in group1 and group2, group is a array of voxelizations"""
    
    g1 = self.readDataGroup(group1);  
    g2 = self.readDataGroup(group2);  
    
    pcoeffs, pvals = stats.pearsonr(g1, g2);

    return pcoeffs, pvals;

def colorPearsonsCoeffs(pcoeffs, cutoff = 0.3):
    
    pcoeffsc = pcoeffs.copy();
    
    pos = pcoeffs > 1-cutoff;
    neg = pcoeffs < -1+cutoff;
    
    ds = pcoeffs.shape + (2,);
    pvc = numpy.zeros(ds);
    
    for i in range(2):
        pvc[pos, i] = pcoeffsc[pos] * [1,0][i];
        
    for i in range(2):
        pvc[neg, i] = pcoeffsc[neg] * [0,-1][i];
...
# needs clean up
def countPointsGroupInRegions(pointGroup, labeledImage = lbl.DefaultLabeledImageFile, intensityGroup = None,
                              intensityRow = 0, returnIds = True, returnCounts = False, collapse = None):
