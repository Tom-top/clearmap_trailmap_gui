...
def labelPoints(points, labeledImage = None, level = None, collapse = None):
    """Label points according to the annotationin the labeled image file
    
    Arguments:
      points: nx3 array of point coordinates
      labeledImage: the labeled image file or array
      level(int or None): maximal level to include
      collapse (int or None): level to which to collapse
    
    Returns:
      array: label of the points
    
    """
    
    x = np.array(points[:,0], dtype = int);
    y = np.array(points[:,1], dtype = int);
    z = np.array(points[:,2], dtype = int); 
    ...
    label = np.full(nPoints, np.nan, dtype = labelImage.dtype);

    valid = np.logical_and(x >= 0, x < dsize[0]);
    valid = np.logical_and(valid, np.logical_and(y >= 0, y < dsize[1]));
    valid = np.logical_and(valid, np.logical_and(z >= 0, z < dsize[2]));
    
    label[valid] = labelImage[x[valid], y[valid], z[valid]];
    
    if collapse is None:
        pointLabels = labelAtLevel(label, level);
    else:
        pointLabels = labelAtCollapse(label);
...
def countPointsInRegions(points, labeledImage = DefaultLabeledImageFile, intensities = None, intensityRow = 0, level= None, allIds = False, sort = True, returnIds = True, returnCounts = False, collapse = None):
    ...
    if allIds:
        lla = np.setdiff1d(Label.ids, ll);
        ll  = np.hstack((ll, lla));
        cc  = np.hstack((cc, np.zeros(lla.shape, dtype = cc.dtype)));
        if not cci is None:
            cci = np.hstack((cci, np.zeros(lla.shape, dtype = cc.dtype)));
        
    
    #cc = np.vstack((ll,cc)).T;
    if sort:
        ii = np.argsort(ll);
        cc = cc[ii];
        ll = ll[ii];
...
def makeColorAnnotations(filename, labeledImage = None):
    ...
    lr = np.zeros(dsize, dtype = np.uint8);
    lg = lr.copy();
    lb = lr.copy();
...
def test():
    ...
    ll, cc = self.countPointsInRegions(points, intensities);
