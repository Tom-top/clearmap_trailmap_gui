...
def resampleXY(source, dataSizeSink, sink = None, interpolation = 'linear', out = sys.stdout, verbose = True, crop = None):
    ...
    if crop :
        ...
        #print(crop["x"][0],crop["x"][1],crop["y"][0],crop["y"][1])
        data = data[ crop["x"][0]:crop["x"][1], crop["y"][0]:crop["y"][1] ];
...
def _resampleXYParallel(arg):
    ...
    fileSource, fileSink, dataSizeSink, interpolation, ii, nn, crop = arg; #verbose
    
    pw = ProcessWriter(ii);
    #if verbose:
        #pw.write("resampleData: resampling in XY: image %d / %d" % (ii, nn))
    
    data = np.squeeze(io.readData(fileSource, z = (ii, ii+1)));
    resampleXY(data, sink = fileSink, dataSizeSink = dataSizeSink, interpolation = interpolation, out = pw, crop = crop); #, verbose = verbose




def resampleData(source, sink = None,  orientation = None, dataSizeSink = None, crop = None, resolutionSource = (4.0625, 4.0625, 3), resolutionSink = (25, 25, 25), 
    ...
    if crop :
        ...
        dataSizeSource = (crop["x"][1]-crop["x"][0],crop["y"][1]-crop["y"][0],dataSizeSource[2])
...
#    return
    ...
    for i in range(nZ):
        argdata.append( (source, os.path.join(processingDirectory, 'resample_%04d.tif' % i), dataSizeSinkI, interpolation, i, nZ, crop) );  
        #print argdata[i]
    ...
    data = io.readData(fn);
    zImage = np.zeros((dataSizeSinkI[0], dataSizeSinkI[1], nZ), dtype = data.dtype);    
    for i in range(nZ):
    ...
    resampledData = np.zeros(dataSizeSinkI, dtype = zImage.dtype);
...
def resampleDataInverse(sink, source = None, dataSizeSource = None, orientation = None, resolutionSource = (4.0625, 4.0625, 3), resolutionSink = (25, 25, 25), 
    ...
    resampledDataXY = np.zeros((dataSizeSinkI[0], dataSizeSinkI[1], dataSizeSource[2]), dtype = resampledData.dtype);
    #print(resampledDataXY.shape)
    ...
    if io.isFileExpression(source, check = False):
        files = source;
    else:
        if processingDirectory == None:
            processingDirectory = tempfile.mkdtemp();   
            
        files = os.path.join(processingDirectory, 'resample_\d{4}.tif');
    ...
    argdata = [];
    for i in range(nZ):
        argdata.append( (sink, os.path.join(processingDirectory, 'resample_%04d.tif' % i), dataSizeSource, interpolation, i, nZ, None) );  
        #argdata.append((source, fl.fileExpressionToFileName(files, i), dataSizeSource, interpolation, i, nZ, None)); 
    
    print(argdata[0])
    
    if processes == 'serial':
      [_resampleXYParallel(a) for a in argdata];  
    else:
      #print(argdata)
      pool = multiprocessing.Pool(processes=processes);
      pool.map(_resampleXYParallel, argdata);
...
import ClearMap.IO.IO_Future as iof

def resampleXYNew(source, dataSizeSink, sink = None, interpolation = 'linear', out = sys.stdout, verbose = True):
    """Resample a 2d image slice
    ...
    # note: cv2.resize reverses x-Y axes
    interpolation = fixInterpolation(interpolation)
    resampledData = cv2.resize(data,  (dataSizeSink[1], dataSizeSink[0]), interpolation = interpolation);
    
    return iof.writeData(sink, resampledData);
    
    

def _resampleXYParallelNew(arg):
    """Resampling helper function to use for parallel resampling of image slices"""
    
    source, sink, dataSizeSink, interpolation, ii, nn, verbose = arg;
    ...
    if isinstance(source, str):
      data = np.squeeze(io.readData(source, z = (ii, ii+1)));
    elif isinstance(source, int):
      data = smm.get(source)[:,:,ii];
       
    if isinstance(sink, int):
      sink = smm.get(sink)[:,:,ii];
    
    resampleXYNew(data, sink = sink, dataSizeSink = dataSizeSink, interpolation = interpolation, out = pw, verbose = verbose);
    


def resampleDataNew(source, sink = None,  orientation = None, dataSizeSink = None, resolutionSource = (4.0625, 4.0625, 3), resolutionSink = (25, 25, 25), 
                    processes = 1, cleanup = True, verbose = True, interpolation = 'linear', **args):
    """Resample data of source in resolution and orientation
    ...
    zImageId = smm.create((dataSizeSinkI[0], dataSizeSinkI[1], nZ), dtype = io.dataType(source));    
    zImage   = smm.get(zImageId);
    
    pool = smm.multiprocessing.Pool(processes=processes);
    argdata = [];
    
    if isinstance(source, np.ndarray):
      source = smm.asShared(source);
      source = smm.insert(source);
      
    for i in range(nZ):
        argdata.append((source, zImageId, dataSizeSinkI, interpolation, i, nZ, verbose) );  
        #print argdata[i]
    pool.map(_resampleXYParallelNew, argdata);
...
def _test():
    ...
    data = res.resampleData(fn, sink = None, resolutionSource = (1,1,1), orientation = (1,2,3), resolutionSink = (10,10,2));
    print(data.shape)
    io.writeData(outfn, data)   

    data = res.resampleData(fn, sink = None, dataSizeSink = (50,70,10), orientation = (1,2,3));
    print(data.shape)
    io.writeData(outfn, data)   


    dataSizeSource, dataSizeSink, resolutionSource, resolutionSink = res.resampleDataSize(dataSizeSource = (100,200, 303), dataSizeSink = None, 
    ...
    pr = res.resamplePoints(points, dataSizeSource = fn, dataSizeSink = (50,70,10), orientation = (1,2,3))
    print(pr)

    pri = res.resamplePointsInverse(pr, dataSizeSource = fn, dataSizeSink = (50,70,10), orientation = (-1,2,3))
    print(pri)


    result = res.resampleDataInverse(outfn, os.path.join(basedir, 'Test/Data/OME/resample_\d{4}.ome.tif'), dataSizeSource = fn);
    print(result)
