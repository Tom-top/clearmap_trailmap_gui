...
def LaunchClearMap(**kwargs) :
    ...
    copyfile(os.path.join(kwargs["clearmapDir"],"Parameters_Raw.py"),os.path.join(kwargs["clearmapDir"],"Parameters.py"))
            ...
            if os.path.isdir(path2Folder) : 
                ...
                if exp == kwargs["experiment"] : 
                  ...
                  if kwargs["operations"][folder] != all_false:
                        ...
                        if os.path.isdir(path2Subfolder) :
                            ...
                            if len(timeStamp.split('-')) == 3 :
                              ...
                              if dataType in ut.cfosPrefix :
                                  ...
                                  print(ut.coloredMessage('[INFO] Current Datadir is : %s'%(datadir),'darkgreen'))
                                  ...
                                  print(ut.coloredMessage('[INFO] %d %s files have been detected'%(nCFosFiles,dataType),'darkgreen'))
                                  cfos = True
                                  ...
                                  print(ut.coloredMessage('[INFO] Current Autodir is : %s'%(autodir),'darkgreen'))
                                  ...
                                  print(ut.coloredMessage('[INFO] %d %s files have been detected \n'%(nAutoFiles,dataType),'darkgreen'))
                                  auto = True
                        ...
                        with open(os.path.join(kwargs["clearmapDir"],'Parameters.py')) as old_file:
                            ...
                            for n,line in enumerate(old_file) :
                                ...
                                if n == backgroundThreshPos :
                                    ...
                                    new_file.write("cFosDetectionRange = {{ 'x' : {0}, 'y' : {1}, 'z' : {2} }}; \n".format(kwargs["cfosX"][folder],\
                    ...
                    os.remove(os.path.join(kwargs["clearmapDir"],'Parameters.py'));
                    os.rename(os.path.join(kwargs["clearmapDir"],"Temp/Parameters.py"),os.path.join(kwargs["clearmapDir"],'Parameters.py'));
...
def launchPipeline(folderOperation,datadir,**kwargs) :
    ...
    if folderOperation[0] == 'T' :
        ...
        importfile = st.xmlImportFile(Parameters.cFosfile,xmlImportFile=os.path.join(Parameters.workdir,'TeraStitcher_import.xml'));
        ...
        alignfile = st.alignData(importfile, search = (48,59,8),xmlResultFile = os.path.join(datadir\
                                 ,'TeraStitcher_align.xml'),subRegion=((all, all),(all,all),(850, 900)));
...
#        points, intensities = thresholdPoints(points, intensities, threshold = Parameters.thresholdPointParameter["threshold"], row = (3,3));
...
#        data = plot.overlayPoints(Parameters.ResamplingParametercFos["source"], pointSource, pointColor = None, **Parameters.cFosDetectionRange);
#        io.writeData(os.path.join(cellsCheckFolder, 'cells_check.tif'), data);
        ...
        points, intensities = io.readPoints(Parameters.ImageProcessingParameter["sink"]);
        points, intensities = thresholdPoints(points, intensities, threshold = Parameters.thresholdPointParameter["threshold"], row = (3,3)); #200,9000#100,900
        ...
        points = transformPoints(points, transformDirectory = Parameters.ChannelsAlignmentParameter["resultDirectory"], indices = False, resultDirectory = None, binary = False);
        points = transformPoints(points, transformDirectory = Parameters.TemplateAlignmentParameter["resultDirectory"], indices = False, resultDirectory = None, binary = False);
        ...
        vox = voxelize(points, Parameters.TemplateFile, **Parameters.voxelizeParameter); #TemplateFile TemplateFileNrrd
        
        if Parameters.analysisDir == None :
            io.writeData(os.path.join(Parameters.workdir, 'cells_heatmap.tif'), vox.astype('int32'));
        else :
            io.writeData(os.path.join(Parameters.analysisDir, 'cells_heatmap_{0}.tif'.format(Parameters.mouseNumber)), vox.astype('int32'));
